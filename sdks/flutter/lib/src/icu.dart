...
/// ICU 인자 이름 1개 이상.
const icuArgName = '[^' + _notArgNameChar + ']+';

/// 인자 타입 키워드(ICU simpleArg/complexArg의 두 번째 자리).
const icuArgType = 'plural|selectordinal|select|number|date|time|spellout|ordinal|duration';

/// 여는 중괄호 + 이름(+ 타입) 스캔 — 서명 수집용.
final icuArgScan =
    RegExp(r'\{\s*(' + icuArgName + r')\s*(?:,\s*(' + icuArgType + r'))?');

/// 타입 없는 단순 인자 — 치환 대상.
final icuSimpleArg = RegExp(r'\{\s*(' + icuArgName + r')\s*\}');
