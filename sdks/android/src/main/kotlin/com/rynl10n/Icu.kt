...
internal object Icu {
    ...
    private const val NOT_ARG_NAME_CHAR =
        "\\u0009-\\u000D\\u0020-\\u002F\\u003A-\\u0040\\u005B-\\u005E\\u0060\\u007B-\\u007E\\u0085\\u00A1-\\u00A7" +
        "\\u00A9\\u00AB-\\u00AC\\u00AE\\u00B0-\\u00B1\\u00B6\\u00BB\\u00BF\\u00D7\\u00F7" +
        "\\u200E-\\u2029\\u2030-\\u203E\\u2041-\\u2053\\u2055-\\u205E\\u2190-\\u245F\\u2500-\\u2775" +
        "\\u2794-\\u2BFF\\u2E00-\\u2E7F\\u3001-\\u3003\\u3008-\\u3020\\u3030\\uFD3E-\\uFD3F\\uFE45-\\uFE46"
    ...
    /** ICU 인자 이름 1개 이상. */
    const val ARG_NAME = "[^" + NOT_ARG_NAME_CHAR + "]+"

    /** 인자 타입 키워드(ICU simpleArg/complexArg의 두 번째 자리). */
    const val ARG_TYPE = "plural|selectordinal|select|number|date|time|spellout|ordinal|duration"

    /** 여는 중괄호 + 이름(+ 타입) 스캔 — 서명 수집용. */
    const val ARG_SCAN = "\\{\\s*(" + ARG_NAME + ")\\s*(?:,\\s*(" + ARG_TYPE + "))?"

    /** 타입 없는 단순 인자 — 치환 대상. */
    const val SIMPLE_ARG = "\\{\\s*(" + ARG_NAME + ")\\s*\\}"

    /** 중괄호 안쪽(name 또는 name, type ...) 선두 매칭 — 변환기용. */
    const val INNER_ARG = "^(" + ARG_NAME + ")\\s*(?:,\\s*([a-z]+))?"
}
