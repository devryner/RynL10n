...
enum Icu {
    ...
    /// ICU 인자 이름 1개 이상.
    static let argName = #"[^"# + notArgNameChar + #"]+"#

    /// 인자 타입 키워드(ICU simpleArg/complexArg의 두 번째 자리).
    static let argType = #"plural|selectordinal|select|number|date|time|spellout|ordinal|duration"#

    /// 여는 중괄호 + 이름(+ 타입) 스캔 — 서명 수집용.
    static let argScan = #"\{\s*("# + argName + #")\s*(?:,\s*("# + argType + #"))?"#

    /// 타입 없는 단순 인자 — 치환 대상.
    static let simpleArg = #"\{\s*("# + argName + #")\s*\}"#

    /// 중괄호 안쪽(name 또는 name, type ...) 선두 매칭 — 변환기용.
    static let innerArg = #"^("# + argName + #")\s*(?:,\s*([a-z]+))?"#
}
