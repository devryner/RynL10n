...
스니펫(복사), 도구 목록, Origin 정책을 보여주고, admin이면 **토큰 발급**까지 여기서 끝난다.

**대시보드가 MCP 표면을 대신 호출하지는 않는다** — 배포 플레인을 링크로만 노출하는 것과 같은
성격이다. 여는 것은 관리 API 쪽 조작뿐이다.

**토큰 발급 카드**(admin 전용)는 사용자 · 라벨 · 표면(`mcp` 기본 / `all`) · 역할 상한을 받아
`POST /users/{id}/tokens` **한 라우트**로 보낸다 — 사용자 관리(프로젝트 목록)의 행 안 발급과 같은
경로이고, 여기서 규칙을 다시 만들지 않는다. 이 화면에 둔 이유는 **여기가 그 토큰을 쓰는 자리**여서다:
사람 계정을 다루는 표 안에서는 "에이전트·CI를 붙인다"는 작업이 셀렉트 두 개로 흩어져 보이지 않는다.
표면 기본값은 최소 권한인 `mcp`이고, 고르는 자리에서 차이를 말해 준다 — **CI 빌드 플러그인은 MCP가
아니라 관리 API(스냅샷 fetch)를 부르므로 `mcp` 토큰을 넣으면 403이고, 그 실패는 다음 빌드 로그에서야
보인다**. 비활성 사용자는 셀렉트에서 빠진다(인증에서 막히는 토큰을 만들어 둘 이유가 없다).
발급 직후 평문은 1회 노출 카드에 뜨고, **표면이 `mcp`일 때만** 붙이는 설정 스니펫의
`Authorization`에 그대로 들어간다 — 손으로 옮기는 단계가 없어지면 그 값이 클립보드·에디터를 떠도는
시간도 짧아진다. `all`로 발급한 것은 꽂지 않는다: MCP에 붙기는 하지만 CI 시크릿에 넣으려고 뽑은 값이
에이전트 설정에 나타나면 어디에 넣을 값인지가 흐려지므로, 스니펫은 자리표시자로 두고 넣을 곳을
말해 준다. 어느 쪽이든 화면을 떠나면 평문은 사라진다.
같은 카드 아래에 **이 인스턴스의 MCP 전용 토큰 목록 + 폐기**를 둔다: 평문을 잃었을 때 할 수 있는 일은
폐기 → 재발급뿐이라 회전이 이 화면에서 끝나야 한다.
