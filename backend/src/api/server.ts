...
import { publishReleaseJob, releaseChanges, rollbackRelease, RangeConflictError, EmptyReleaseError, NotFoundError } from "../pipeline/publish.ts";
...
const routes: Route[] = [
  ...
  // publish — Maintainer+ (202 잡 / 409 충돌 / 422 빈 릴리스). 잡·지표·알림까지 publishReleaseJob이 감싼다(MCP 도구와 공유).
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      const code = err.name === "AuthError" ? (status === 401 ? "unauthorized" : "forbidden")
        ...
        : err instanceof EmptyReleaseError ? "empty_release"
