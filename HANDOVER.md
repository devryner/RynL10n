...
- **로드맵 M0~M4 전 마일스톤 완주 + 파리티 마감 + 대시보드 + 4개 플랫폼 앱 적용 경로.**
  커밋 79개(`1c0e225`~`a8ce4a3` — 이 줄을 갱신하는 문서 커밋 자신은 세지 않으므로 항상 한 칸 뒤처진다.
...
- **테스트 511개 전부 통과** — TS 참조 75 · 백엔드 208 · mcp-stdio 37 · Web 33 · iOS 50 · Android 54 · Flutter 54.
  ...
  아래 "MCP 도구 표면" 절.)
  (**소비자 스모크는 이 511개 밖이다** — 저장소 소스가 아니라 레지스트리 게시본을 보므로 CI가 아니라
...
의존성, 태그가 가리키는 커밋이 매니페스트 없는 자리인 경우. 저장소 안에서 도는 511개 테스트는 전부
