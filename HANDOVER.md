...
- **로드맵 M0~M4 전 마일스톤 완주 + 파리티 마감 + 대시보드 + 4개 플랫폼 앱 적용 경로.**
  커밋 50개(`1c0e225`~`471ddf0`). **전부 `origin/main` 머지 완료(PR #1~#10, rebase — 머지 커밋 없는
  선형 이력)** — 미머지 브랜치 없음.
...
- 기획서의 모든 확정 설계가 구현·검증됨. 남은 것은 실제 앱/외부 환경 의존 항목과
  대시보드 확장 3건뿐(아래 "대시보드" 절 · "열려 있는 항목" 참조).
...
**관리 API에는 있으나 UI 진입점이 없는 것**(= 다음 확장 후보, 백엔드는 이미 완성):

| 엔드포인트 | 메모 |
| --- | --- |
| `POST .../translations/{key}/backport` | **백포트의 키 축**(한 키 → 여러 릴리스). 릴리스 축(`releases/{r}/keys`)은 UI에 있고 둘 다 결국 `repo.addReleaseKey`를 부른다 — 축만 다르다. 부분 실패 207도 아직 표면화 안 됨 |
| `GET .../releases/{r}/keys` · `/snapshot` | 쓰기는 되는데 카탈로그·스냅샷 **읽기** 화면이 없다 |
