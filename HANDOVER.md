...
- **로드맵 M0~M4 전 마일스톤 완주 + 파리티 마감 + 대시보드 + 4개 플랫폼 앱 적용 경로.**
  커밋 99개(`1c0e225`~`f69eab3` — 이 줄을 갱신하는 문서 커밋 자신은 세지 않으므로 항상 한 칸 뒤처진다.
...
- **테스트 549개 전부 통과** — TS 참조 78 · 백엔드 233 · mcp-stdio 38 · Web 33 · iOS 51 · Android 62 · Flutter 54.
  (2026-09-16 **`release_applied` 텔레메트리 이벤트** — `src/`와 4개 SDK가 같이 바뀌므로 5개 컴포넌트 전부
  재실행. 골든 벡터는 텔레메트리를 담지 않아 재생성 불필요이고, 재생성해도 무변경을 확인했다.
  아래 커밋 지도 참조.)
  (2026-09-12 **publish 빈 릴리스 가드** · 2026-09-11 **MCP 쓰기 도구 2종**으로 백엔드 210 → 233.
  둘 다 `src/`·`fixtures/`·`sdks/` 무변경이라 재실행은 TS 참조·백엔드·mcp-stdio로 한정했다.)
  (2026-08-28 **Android bake CLI에 `--descriptions`** — 변환기·골든은 이미 맞았고 CLI 배선만 빠져
  있었다. `src/`·`fixtures/`가 무변경이라 Android·mcp-stdio만 재실행.)
  (2026-08-28 **앱 개발자용 stdio MCP 서버** — `bake_preview`·`lockfile_status` 두 도구. `src/`·
  `fixtures/`·`sdks/`가 무변경이라 SDK 재실행 없이 TS 참조·백엔드·mcp-stdio만 돌렸다.
  아래 "stdio MCP 서버" 절.)
  ...
  아래 "MCP 도구 표면" 절.)
  (**소비자 스모크는 이 549개 밖이다** — 저장소 소스가 아니라 레지스트리 게시본을 보므로 CI가 아니라
...
의존성, 태그가 가리키는 커밋이 매니페스트 없는 자리인 경우. 저장소 안에서 도는 549개 테스트는 전부
...
- **출시 전 변경사항 + 카탈로그·스냅샷 읽기**(2026-09-01) — 릴리스 탭의 "변경사항". 현재 DB
  카탈로그를 마지막 게시본과 비교해 추가·수정·삭제 합계와 키·로케일별 이전/게시 후 값을 보여준다.
  같은 릴리스 재게시는 현재 overlay(롤백 반영), 한 번도 게시하지 않은 신규 릴리스는 같은 매칭 전략의
  직전 릴리스가 기준이다. 서버 라우트 `GET /projects/{p}/releases/{r}/changes`는 실제 publish와 같은
  `buildSnapshot`·`buildDelta`를 재사용해 미리보기와 실제 delta의 규칙이 갈리지 않는다. 기존 현재
  카탈로그·스냅샷 JSON도 접어서 계속 볼 수 있다. 읽기 라우트라 **쓰기 게이트 앞에 두어 viewer도
  닿는다**(다른 릴리스 작업 버튼은 여전히 manage 전용).
...
Origin 정책을 보여주고, admin이면 **토큰 발급**(2026-09-02)까지 여기서 끝난다 — 사용자 · 라벨 ·
표면(`mcp` 기본 / `all`) · 역할 상한을 받아 사용자 관리와 **같은 `POST /users/{id}/tokens`** 로 보낸다.
이 화면에 둔 이유는 여기가 그 토큰을 **쓰는** 자리여서다(사람 계정 표 안에서는 그 작업이 셀렉트 두 개로
흩어져 보이지 않는다). 표면을 고르는 자리에서 차이를 말해 준다 — **CI 빌드 플러그인은 MCP가 아니라
관리 API를 부르므로 `mcp` 토큰을 넣으면 403이고 다음 빌드 로그에서야 보인다**. 발급된 평문은 1회 노출
카드에 뜨고 **표면이 `mcp`일 때만 설정 스니펫의 `Authorization`에 삽입**된다(`all`은 넣을 자리가 CI
시크릿이라 자리표시자로 남긴다). MCP 전용 토큰 목록·폐기도 같은 화면에 있다(회전).
**대시보드가 MCP 표면 자체를 부르지는 않는다.** 도구 목록은 `GET /mcp/tools`에서 오는데,
...
### ③ 앱 개발자 로컬 stdio MCP 서버 — **2026-08-28 착수(도구 2종 착지)**

`bake_preview`·`lockfile_status`가 들어왔다(아래 "stdio MCP 서버" 절). 남은 것은
...
### `lockfile_status` — 지금 상태와 **앱이 실제로 집는 파일**

`bake_preview`가 다음 빌드를 본다면 이건 지금을 본다. lockfile은 빌드가 의도한 릴리스·base의 유일한
기록이라(런타임은 안 읽는다 — 스냅샷 자신이 들고 있다) "의도한 것"과 "디스크에 있는 것"이 어긋났는지
볼 자리도 여기뿐이다.

**만들면서 드러난 것이 이 도구의 존재 이유가 됐다.** `--stable-name` 없이 구우면
`snapshot-<base>.json`이 빌드마다 **쌓이는데**, 그때 로더는 Android가 `minByOrNull { it.name }`
(**파일명 최소값**이지 최신이 아니다), iOS가 `bundle.urls(...).first(where:)`(**순서 미보장**)를 집는다.
둘 다 조용하다 — 앱은 스테일 카탈로그를 들고 멀쩡히 돌고 개발자는 왜 새 번역이 안 보이는지 모른다.
lockfile의 base와 대조해야만 드러나고, 그래서 진단 코드에 `loads_stale_bundle`이 있다(lockfile이
가리키는 번들이 바로 옆에 있는데 로더가 그것을 안 집는 경우). 코드 9종은 조기 반환 지점과 1:1이고
케이스마다 원인을 실제로 만들어 대조한다.

**탐색 순서가 SDK 로더와 같아야 한다**는 것이 이 도구의 불변식이다. 어긋나면 "앱이 이걸 집는다"고
답한 파일과 앱이 실제로 집는 파일이 달라지는데, 그건 진단 도구가 낼 수 있는 최악의 오답이다.
...
- **실제 앱 통합**: **Xcode 앱 타깃의 플러그인 경로는 2026-09-10에 닫혔다** — 실제 앱(RynDevice)을 로컬
...
- **내용해시 번들이 쌓이면 로더가 스테일을 집을 수 있다**(2026-08-28 `lockfile_status`를 만들다 확인,
  ...
  iOS `Snapshot.bakedURL`은 `bundle.urls(...)`의 `first(where:)`라 **순서가 보장되지 않는 것**을
  집는다. 앱은 스테일 카탈로그를 들고 조용히 돌고, 기기마다 다른 것을 볼 수도 있다.
  **고치는 방향은 정해져 있다** — 로더가 옆의 `rynl10n.lock`을 먼저 읽고 그 `base`와 맞는 스냅샷을
  고르면 된다(lockfile은 이미 그 자리에 굽는다). 다만 Swift·Kotlin 두 로더 + 각 테스트가 한 묶음이고,
  실무 배치는 대개 `--stable-name`이라(Gradle 태스크 기본값) 급하지 않다고 봤다. 그때까지는
  `lockfile_status`가 이 상태를 **진단**한다(`stale_candidates`·`loads_stale_bundle`).
...
- **Android bake CLI `--descriptions`** `ccf8b8f`(2026-08-28) — 기획서와 README가 약속한
  `strings.xml` XML 주석이 **Android 빌드에서는 실제로 안 구워지고 있었다.** 변환기도 골든 벡터도
  이미 맞았고 **CLI가 인자를 안 넘긴 것**이 전부였다(플래그 자체가 없었다. iOS에는 있었다).
  `bake_preview`를 만들며 "미리보기는 CLI의 실제 동작을 따라야 한다"를 확인하다 드러났다.
  `loadDescriptions`는 iOS와 허용 형태·실패 정책까지 맞췄다. Gradle `-Pdescriptions=`에는 빈 문자열
  가드를 걸었다.
- **stdio MCP 서버 · `lockfile_status`** `a8ce4a3`(2026-08-28) — "새 번역이 왜 안 보이지"에 답하는
  자리. 만들다 드러난 것이 존재 이유가 됐다: 내용해시 번들이 쌓이면 **Android는 파일명 최소값**
  (최신이 아니다), **iOS는 순서 미보장**인 것을 집어 앱이 스테일 카탈로그를 들고 조용히 돈다.
  진단 9종은 조기 반환 지점과 1:1. 불변식은 **탐색 순서가 SDK 로더와 같을 것** — 어긋나면 "앱이
  이걸 집는다"고 답한 파일과 실제가 달라진다. 로더 쪽 수정은 열려 있는 항목에 근거와 함께 남겼다.
...
- **출시 전 변경사항 뷰** `233c7ae`(2026-09-01) — publish 단추를 누르기 전에 무엇이 나가는지 볼 자리가
  없었다. 실제 publish와 **같은 delta 규칙**으로 마지막 게시본 대비 추가·수정·삭제와 이전/게시 후 값을
  보여 준다. 상세는 위 "대시보드" 절.
- **iOS 빌드 경고 17건 제거** `d3c096c`(2026-09-10, PR #19) — 저장소 테스트는 다 통과하는데 **소비 앱의
  빌드 로그**에만 나타나던 결함이다. `PackagePlugin.Path` 계열이 swift-tools 6.0에서 폐기돼 소비 앱
  빌드마다 경고 14건이 찍혔다 — `URL` API로 옮겼다. `Target.directoryURL`은 tools 6.1부터라 6.0
  매니페스트에서는 못 쓰므로 구체 타입(`SwiftSourceModuleTarget`·`ClangSourceModuleTarget`)으로 받는다.
  **tools-version을 올리면 소비자의 최소 Xcode가 따라 오른다**는 것이 판단 근거다. CLI 인자는
  `path(percentEncoded: false)`로 푼다(퍼센트 인코딩을 두면 공백·한글 경로에서 파일을 못 찾는다).
  나머지 셋은 SDK 소스(`try?` 결과 미사용 둘, bake CLI의 세마포어 캡처). 이때 실제 Xcode 앱 타깃
  플러그인 경로도 처음 확인됐다 — 위 "열려 있는 항목" 참조.
- **로컬 CI 전환** `ed53666`(2026-09-10, PR #20) — `ci.yml`에서 push·PR 트리거를 빼고 같은 검사를
  `tools/ci-local.sh`로 옮겼다(Actions 한도 절약). `workflow_call`(태그 게시 게이트)·`workflow_dispatch`는
  남는다. 상세는 이 문서 맨 위 "로컬 CI 전환 완료" 절.
- **MCP 쓰기 도구 2종** `98f55d3`(2026-09-11) — 읽기만 되던 표면에서 에이전트가 게시·검수까지 잇는다.
  `publish_release`는 **관리 API 라우트와 같은 `publishReleaseJob`을 돈다** — 표면마다 따로 감싸면 MCP로
  게시한 publish가 대시보드 잡 목록과 지표에서 사라진다. `review_translation`은 전이 전에 저장값을
  단일 원천 검증기로 재검사한다(검증을 거치지 않은 경로로 들어온 값이 검수 딱지를 달면 안 된다).
  **역할 상한이 이때 실질이 됐다** — `tools/list`가 capability로 걸러지므로 상한이 곧 도구 목록이다.
  상세는 위 "MCP 도구 표면 › 도구 4종" 절.
- **MCP 화면에서 토큰 발급** `05c8b6e`~`c9a4806`(2026-09-11) — 붙이는 자리와 발급하는 자리가 떨어져
  있었다. 발급은 사용자 관리와 **같은 라우트**를 쓰고(규칙을 두 벌 만들지 않는다), 평문은 **표면이
  `mcp`일 때만** 설정 스니펫에 꽂는다 — `all`은 CI 시크릿용이라 에이전트 설정에 나타나면 어디에 넣을
  값인지가 흐려진다. 브라우저에서 직접 확인하며 결함 둘을 더 잡았다(`620d666`).
- **publish 빈 릴리스 가드** `ec237d4`(2026-09-12, PR #21) — 빈 스냅샷을 게시하면 SDK가 번들을 통째로
  갈아끼워 **그 버전대 앱의 번역이 전부 사라진다**(`⟪key⟫`). 범위 충돌 검사 뒤·자동 상한 닫힘 쓰기
  앞에서 422 `empty_release`로 막으므로 관리 API·MCP·화면이 한 곳에서 막히고, 거절된 publish가 이전
  릴리스의 상한만 닫아 놓고 끝나지 않는다. **직전 게시본의 키가 빠진 것은 막지 않는다** —
  실수와 새 앱 버전에서 걷어낸 키를 서버가 가를 수 없어 `droppedKeys`로 알리기만 한다. 지표에는
  `result="empty"`를 따로 둬 입력 거절이 오류율에 섞이지 않게 했다.
- **`release_applied` 텔레메트리 이벤트** `33b52fc`(2026-09-16, PR #22) — 델타는 overlay가 base와 다를
  때만 manifest에 실린다. 첫 publish·base 롤백·rollout 밖 기기는 `overlay_applied`가 영영 0이라,
  서버에서 **"아무도 안 쓴다"와 "쓴다고 말할 방법이 없다"가 똑같이 생겼다**. 실제로 다운스트림
  대시보드가 한 번만 게시한 릴리스를 "관찰 이래 사용 없음"으로 보관 후보에 올렸고, 보관은 manifest에서
  릴리스를 빼는 일이라 그걸 믿고 누르면 그 버전대 앱의 원격 갱신이 끊긴다. 판정 자리는 swap 하나뿐이라
